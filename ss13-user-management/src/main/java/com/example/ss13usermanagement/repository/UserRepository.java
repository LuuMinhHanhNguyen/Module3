...
public class UserRepository implements IUserRepository{
    ...
    public List<User> selectAllUsers() {
        ...
        try {
            CallableStatement preparedStatement = connection.prepareCall(BaseConnection.SELECT_ALL_USERS);
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()){
                int id = rs.getInt( "id");
                String name = rs.getString("name");
    ...
    public boolean deleteUser(int id) throws SQLException {
        boolean rowDeleted;
        Connection connection = BaseConnection.getConnection();
        CallableStatement preparedStatement = connection.prepareCall(BaseConnection.DELETE_USERS_SQL);
        preparedStatement.setInt(1,id);
    ...
    public boolean updateUser(User user) throws SQLException {
        boolean rowUpdated;
        Connection connection = BaseConnection.getConnection();
        CallableStatement preparedStatement = connection.prepareCall(BaseConnection.UPDATE_USERS_SQL);
        preparedStatement.setString(1,user.getName());
        preparedStatement.setString(2,user.getEmail());
        preparedStatement.setString(3,user.getCountry());
